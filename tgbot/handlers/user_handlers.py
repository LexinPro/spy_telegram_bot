...
def register_handlers(bot: TeleBot):
    ...
    def start_command(message: Message):
        ...
        if not is_user_authorized(user_id):
            register_new_user(user_id, user_name)
    ...
    def edited_gender_callback(call: CallbackQuery):
        ...
        message_id = call.message.id
        user = find_user(user_id)
        user.gender = "Мужской" if call.data == "edit_gender_man" else "Женский"
        save_all_users()
    ...
    @bot.callback_query_handler(func=lambda callback: callback.data == "back_info_command", state='*')
    def back_callback(call: CallbackQuery):
        if call.data == "back_info_command":
            user_id = call.from_user.id
            message_id = call.message.id
            bot.delete_state(user_id, user_id)
